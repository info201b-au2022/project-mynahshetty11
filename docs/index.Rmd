...
#Value 2
max_prop_how_many <- filter(handwash_filtered, handwash_filtered$Percent == max(handwash_filtered$Percent)) %>%
  pull(Country)
...
#Value 4
median_cholera_cases  <- round(median(cholera$Number.of.reported.cases.of.cholera, na.rm =TRUE), digits = 2)

#Value 5
minimum_proportion_handwash <-filter(handwash_filtered, handwash_filtered$Percent == min(handwash_filtered$Percent)) %>%
  pull(Percent)
minimum_proportion_handwash <- minimum_proportion_handwash[1]

#Value 6
min_prop_where <- filter(handwash_filtered, handwash_filtered$Percent == min(handwash_filtered$Percent)) %>%
  pull(Country)
min_prop_where <- length(min_prop_where)
