...
=======
poverty <- read.csv("../data/data_poverty.csv")
>>>>>>> 8bb36d0b8dd87c708813df6786014b9dd3989545
```
## Dynamic Paragraph
...
#Value 1
maximum_proportion_handwash <- filter(handwash_filtered, handwash_filtered$Percent == max(handwash_filtered$Percent)) %>%
  pull(Percent)
maximum_proportion_handwash <- maximum_proportion_handwash[1]

max_prop_where <- filter(handwash_filtered, handwash_filtered$Percent == max(handwash_filtered$Percent)) %>%
  pull(Country)
...
#Value 3
mean_cholera_cases  <- round(mean(cholera$Number.of.reported.cases.of.cholera, na.rm =TRUE), digits = 2)
